...
//devuelve la semana en q estoy
          ...
          $where = "(
                      ...
                      (
                        (  CONCAT( AES_DECRYPT(p.nombre,'{$this->key_hash}'),' ',AES_DECRYPT(p.apellidos,'{$this->key_hash}') ) LIKE  '%".$cadena."%' ) OR 
                        ( AES_DECRYPT(p.email,'{$this->key_hash}') LIKE  '%".$cadena."%' ) OR
                         ( AES_DECRYPT(p.contrasena,'{$this->key_hash}') LIKE  '%".$cadena."%' ) OR
                         ( CONCAT('@',AES_DECRYPT(p.nick,'{$this->key_hash}') )LIKE  '%".$cadena."%' )  OR 
                         ( AES_DECRYPT(p.telefono,'{$this->key_hash}') LIKE  '%".$cadena."%' ) 
                       )
            )";   
...
//////////////////////////////////////////////////////////////
  ...
  public function buscador_detalle_participantes($data){
          ...
          //$this->db->select("r.tienda AS numero_tienda", FALSE);             
          $this->db->select("r.tienda AS numero_tienda", FALSE);   
          



         $this->db->select("( CASE WHEN r.fecha_pc-18000 = 0 THEN '' ELSE DATE_FORMAT(FROM_UNIXTIME(r.fecha_pc-18000),'%d-%m-%Y %T') END ) AS fecha_pc", FALSE); 
              ...
              if ( $result->num_rows() > 0 ) {
                  ...
                  foreach ($result->result() as $row) {
                               ...
                               $dato[]= array(
                                     ...
                                     3=>$row->cadena, 
                                     4=>$row->ticket, 
                                     5=>$row->fecha_compra.'<br/><span style="color:red;"> '.$row->fecha_pc.'</span>', 
                                     6=>intval($row->monto), 
                                     7=>$row->numero_tienda, 
                                     8=>$row->producto, 
                                     9=>$row->litraje, 
...
//devuelve la semana en q estoy
 ...
 public function exportar_participantes_detalle($data){
          ...
          $this->db->select("AES_DECRYPT(p.telefono,'{$this->key_hash}') AS telefono", FALSE);    //sino es p.celular
          $this->db->select("( CASE WHEN p.fecha_nac = 0 THEN '' ELSE DATE_FORMAT(FROM_UNIXTIME(p.fecha_nac),'%d-%m-%Y') END ) AS fecha_nacimiento", FALSE); 
         $this->db->select("p.id_facebook AS facebook", FALSE);
      ...
      //devuelve la semana en q estoy
          ...
          //$this->db->select("AES_DECRYPT(r.ticket,'{$this->key_hash}') AS ticket", FALSE);      
           ...
           $this->db->select("Concat('T-',AES_DECRYPT(r.ticket,'{$this->key_hash}')) AS ticket", FALSE);  
